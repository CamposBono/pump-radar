import os, requests, time, threading
from datetime import datetime, timedelta
from flask import Flask
import pytz

T = os.environ.get("TELEGRAM_TOKEN")
C = os.environ.get("TELEGRAM_CHAT_ID")
Z = pytz.timezone("America/Argentina/Buenos_Aires")
P = [("BTC-USD","BTC"),("ETH-USD","ETH"),("SOL-USD","SOL"),("XRP-USD","XRP"),("ADA-USD","ADA")]
D = {"l":0,"s":0,"f":""}
H = {}
app = Flask(__name__)

@app.route("/")
def home():
    return "Pump Radar v10 activo", 200

def send(t):
    try:
        requests.post(
            f"https://api.telegram.org/bot{T}/sendMessage",
            json={"chat_id": C, "text": t, "parse_mode": "Markdown"},
            timeout=10
        )
    except:
        pass

def ohlc(par, iv=60, limite=50):
    try:
        seg = {60:3600, 240:14400, 1440:86400}.get(iv, 3600)
        r = requests.get(
            f"https://api.exchange.coinbase.com/products/{par}/candles",
            params={"granularity": seg, "limit": limite},
            timeout=8
        )
        if r.ok:
            return list(reversed(r.json()))
    except:
        pass
    return []

def fp(x):
    return f"${x:,.2f}" if x > 100 else f"${x:,.4f}" if x > 1 else f"${x:,.6f}"

def sesion():
    hm = datetime.now(Z).hour * 60 + datetime.now(Z).minute
    if 10*60+30 <= hm < 17*60:    return "ny",   65
    if 4*60     <= hm < 10*60+30: return "eu",   62
    if hm >= 21*60 or hm < 3*60:  return "asia", 68
    return "off", 65

def sesgo_diario(par):
    v = ohlc(par, 1440, 15)
    if len(v) < 10: return "n"
    Hi = [float(x[2]) for x in v[-10:]]
    Lo = [float(x[3]) for x in v[-10:]]
    Cl = [float(x[4]) for x in v[-10:]]
    ma = sum(Cl) / len(Cl)
    if Hi[-1] < Hi[-3] and Lo[-1] < Lo[-3] and Cl[-1] < ma: return "b"
    if Hi[-1] > Hi[-3] and Lo[-1] > Lo[-3] and Cl[-1] > ma: return "a"
    return "n"

def compresion_h1(par):
    v = ohlc(par, 60, 25)
    if len(v) < 20: return 0
    Hi = [float(x[2]) for x in v[-20:]]
    Lo = [float(x[3]) for x in v[-20:]]
    # Rango actual — promedio ultimas 5 velas
    ra = sum((Hi[i]-Lo[i]) for i in range(-5,0)) / 5
    # Rango historico — promedio 15 velas anteriores
    rh = sum((Hi[i]-Lo[i]) for i in range(-20,-5)) / 15
    if rh < 0.0001: return 0
    ratio = ra / rh
    # Score segun compresion
    if ratio < 0.3:  return 100
    if ratio < 0.5:  return 80
    if ratio < 0.65: return 60
    if ratio < 0.8:  return 40
    return 0

def compresion_decreciente(par):
    # Detecta si la compresion esta achicandose vela a vela — anticipacion real
    v = ohlc(par, 60, 10)
    if len(v) < 6: return False
    rangos = [float(v[i][2]) - float(v[i][3]) for i in range(-6, 0)]
    # Al menos 4 de las ultimas 6 velas con rango decreciente
    decreciente = sum(1 for i in range(1, len(rangos)) if rangos[i] < rangos[i-1])
    return decreciente >= 4

def zona_h4(par, p):
    # Maximo y minimo de los ultimos 5 dias en H4
    v = ohlc(par, 240, 30)
    if len(v) < 20: return False, "ninguna", 0.0
    Hi      = [float(x[2]) for x in v[-30:]]
    Lo      = [float(x[3]) for x in v[-30:]]
    max_h4  = max(Hi)
    min_h4  = min(Lo)
    d_max   = abs(p - max_h4) / max(p, 0.001) * 100
    d_min   = abs(p - min_h4) / max(p, 0.001) * 100
    if d_max < 2.0: return True, "resistencia", round(d_max, 2)
    if d_min < 2.0: return True, "soporte",     round(d_min, 2)
    return False, "ninguna", 0.0

def sl_estructural(par, tipo):
    v = ohlc(par, 60, 12)
    if len(v) < 10: return 0
    Hi = [float(x[2]) for x in v[-10:]]
    Lo = [float(x[3]) for x in v[-10:]]
    return min(Lo) if tipo == "long" else max(Hi)

def ana(par, sym, forzar=False):
    ses, umbral_ses = sesion()
    if ses == "off" and not forzar: return None

    v1h = ohlc(par, 60, 25)
    if len(v1h) < 20: return None

    Cl  = [float(x[4]) for x in v1h[-20:]]
    Vv  = [float(x[5]) for x in v1h[-20:]]
    p   = Cl[-1]

    # Volumen relativo
    vol_avg = sum(Vv[-21:-1]) / max(len(Vv[-21:-1]), 1)
    vr      = Vv[-1] / max(vol_avg, 0.0001)
    if vr < 0.05: vr = Vv[-2] / max(vol_avg, 0.0001)

    c1 = (Cl[-1] - Cl[-2]) / max(Cl[-2], 0.001) * 100
    c4 = (Cl[-1] - Cl[-5]) / max(Cl[-5], 0.001) * 100

    # Movimiento reciente — si ya se movio, llegamos tarde
    if abs(c1) > 1.5: return None

    sd     = sesgo_diario(par)
    sc_c   = compresion_h1(par)
    dec    = compresion_decreciente(par)
    zona_cerca, zona_tipo, zona_dist = zona_h4(par, p)

    # Compresion minima requerida
    if sc_c < 40: return None

    for tipo in ["long", "short"]:
        k = f"{sym}_{tipo}"
        if k in H and datetime.now(Z) - H[k] < timedelta(hours=4): continue

        # Momentum — no entrar si ya se movio en esa direccion
        if tipo == "long"  and c1 < 0 and c4 < 0: continue
        if tipo == "short" and c1 > 0 and c4 > 0: continue
        if tipo == "long"  and c4 > 3:  continue
        if tipo == "short" and c4 < -3: continue

        # Zona — filtro de direccion
        if zona_cerca:
            if zona_tipo == "resistencia" and tipo == "long":  continue
            if zona_tipo == "soporte"     and tipo == "short": continue

        a_favor = (sd == "b" and tipo == "short") or (sd == "a" and tipo == "long")
        contra  = (sd == "b" and tipo == "long")  or (sd == "a" and tipo == "short")

        # SL estructural
        sl_est = sl_estructural(par, tipo)
        if sl_est == 0: continue
        sl_pct = abs(p - sl_est) / max(p, 0.001) * 100
        if sl_pct < 0.3 or sl_pct > 5: continue

        # TP diferenciado por sesgo
        tp_ratio = 2.5 if a_favor else 1.8
        tp_pct   = sl_pct * tp_ratio
        tp1      = p * (1 + tp_pct/100) if tipo == "long" else p * (1 - tp_pct/100)

        # SCORE
        sc  = 40
        sc += int(sc_c * 0.35)           # compresion: hasta 35pts
        sc += 10 if dec else 0           # compresion decreciente: +10pts anticipacion
        sc += 15 if a_favor else -10 if contra else 0
        if zona_cerca:
            sc += 15 if (zona_tipo == "soporte" and tipo == "long") or \
                        (zona_tipo == "resistencia" and tipo == "short") else 5
        sc += 10 if vr > 1.5 else 5 if vr > 0.8 else -10 if vr < 0.3 else 0
        sc += 5  if tipo == "long" and 0 < c4 <= 3 else \
              5  if tipo == "short" and -3 <= c4 < 0 else 0
        sc += 5 if ses == "ny" else 3 if ses == "eu" else 1 if ses == "asia" else 0
        if vr < 1: sc = int(sc * 0.92)
        sc = min(sc, 100)

        umbral = umbral_ses if not forzar else 58
        if contra: umbral = max(umbral, 75)
        if sc < umbral: continue

        H[k] = datetime.now(Z)

        em       = "🟢" if tipo == "long" else "🔴"
        sdt      = {"a":"📈Alc","b":"📉Baj","n":"➡️Neu"}.get(sd, "")
        zona_tag = f"{zona_tipo}({zona_dist}%)" if zona_cerca else ""
        dec_tag  = "Comp↓✅" if dec else ""
        apal     = 5 if sc >= 85 else 3

        tags = [sdt, f"Comp:{sc_c}pts"]
        if dec_tag:  tags.append(dec_tag)
        if zona_tag: tags.append(zona_tag)
        tags.append(f"Vol:{vr:.1f}x|{ses.upper()}")

        return {
            "sym": sym, "p": fp(p), "sc": sc, "tipo": tipo,
            "c1": c1, "c4": c4, "vr": vr,
            "tp": fp(tp1), "sl": fp(sl_est),
            "tpp": round(tp_pct, 1), "slp": round(sl_pct, 1),
            "apal": apal, "em": em, "tags": tags
        }

    return None

def dbg(par, sym):
    v1h = ohlc(par, 60, 25)
    if not v1h or len(v1h) < 20: return f"⚠️{sym}:sin datos"
    Cl  = [float(x[4]) for x in v1h[-20:]]
    Vv  = [float(x[5]) for x in v1h[-20:]]
    p   = Cl[-1]
    vol_avg = sum(Vv[-21:-1]) / max(len(Vv[-21:-1]), 1)
    vr      = Vv[-1] / max(vol_avg, 0.0001)
    c1 = (Cl[-1] - Cl[-2]) / max(Cl[-2], 0.001) * 100
    c4 = (Cl[-1] - Cl[-5]) / max(Cl[-5], 0.001) * 100
    sd    = sesgo_diario(par)
    sc_c  = compresion_h1(par)
    dec   = compresion_decreciente(par)
    zona_cerca, zona_tipo, zona_dist = zona_h4(par, p)
    ses, _ = sesion()
    sdn  = {"a":"alc","b":"baj","n":"neu"}.get(sd, "?")
    return (f"📊*{sym}*`{fp(p)}`\n"
            f"Diario:`{sdn}` Ses:`{ses}`\n"
            f"Comp H1:`{sc_c}pts` Dec:`{'✅'if dec else'❌'}`\n"
            f"Zona H4:`{zona_tipo}` dist:`{zona_dist}%`\n"
            f"Vol:`{vr:.1f}x`\n"
            f"1h:`{c1:+.1f}%` 4h:`{c4:+.1f}%`")

def run_bg(forzar=False):
    global D
    now = datetime.now(Z)
    hoy = now.strftime("%d/%m")
    if D["f"] != hoy: D = {"l":0,"s":0,"f":hoy}
    if D["l"] >= 4 and D["s"] >= 3: return

    alertas = []
    ls, ss  = [], []

    for par, sym in P:
        v1h = ohlc(par, 60, 25)
        if v1h and len(v1h) >= 20:
            Cl  = [float(x[4]) for x in v1h[-20:]]
            p   = Cl[-1]
            sc_c = compresion_h1(par)
            dec  = compresion_decreciente(par)
            sd   = sesgo_diario(par)
            zona_cerca, zona_tipo, _ = zona_h4(par, p)
            # Pre-alerta — compresion alta sola ya es informacion
            if sc_c >= 70:
                dir_txt  = {"b":"SHORT","a":"LONG"}.get(sd, "⚠️")
                zona_t   = f"cerca {zona_tipo}" if zona_cerca else ""
                dec_t    = "comprimiendo↓" if dec else ""
                alertas.append(
                    f"⚠️*{sym}*`{fp(p)}` Comp:{sc_c}pts {dec_t} {zona_t} →{dir_txt}")

        r = ana(par, sym, forzar)
        if r:
            if r["tipo"] == "long"  and D["l"] < 4: ls.append(r)
            elif r["tipo"] == "short" and D["s"] < 3: ss.append(r)
        time.sleep(0.5)

    ls.sort(key=lambda x: x["sc"], reverse=True)
    ss.sort(key=lambda x: x["sc"], reverse=True)
    tl, ts = ls[:2], ss[:1]

    if not tl and not ts:
        if alertas:
            send("🔍*Sin señales — Pre-alertas:*\n" + "\n".join(alertas))
        else:
            ses, _ = sesion()
            send(f"🔍Sin condiciones | Ses:`{ses}`")
        return

    msg = f"⚡*PUMP RADAR v10—{now.strftime('%H:%M')}ARG*\n_Compresión H1 | Zona H4 | Sesgo diario_\n\n"
    for r in tl + ts:
        signo = "+" if r["tipo"] == "long" else "-"
        msg += (f"{r['em']}*{r['tipo'].upper()}—{r['sym']}*|`{r['sc']}/100`\n"
                f"📍`{r['p']}`|Vol:`{r['vr']:.1f}x`|1h:`{r['c1']:+.1f}%`\n"
                f"🎯`{r['tp']}`({signo}{r['tpp']}%)|🛑`{r['sl']}`(-{r['slp']}%)|⚡`{r['apal']}x`\n"
                f"_{', '.join(r['tags'])}_\n\n")
        if r["tipo"] == "long": D["l"] += 1
        else:                   D["s"] += 1

    msg += f"📊{D['l']}L {D['s']}S|⚠️Experimental"
    send(msg)

def run(forzar=False):
    threading.Thread(target=lambda: run_bg(forzar), daemon=True).start()

def run_debug():
    send("🔬*DEBUG v10 — Comp H1 | Zona H4 | Sesgo diario*")
    for par, sym in P:
        send(dbg(par, sym))
        time.sleep(0.5)
    send("✅Debug completo")

def listen():
    last = 0
    while True:
        try:
            r = requests.get(
                f"https://api.telegram.org/bot{T}/getUpdates",
                params={"offset": last+1, "timeout": 10},
                timeout=15
            )
            if r.ok:
                for u in r.json().get("result", []):
                    last = u["update_id"]
                    t    = (u.get("message") or {}).get("text") or ""
                    if   t == "/start":   send("👋*Pump Radar v10*\n/analizar /resumen /debug /ayuda")
                    elif t == "/analizar":run(forzar=True)
                    elif t == "/resumen": send(f"📊Hoy:{D['l']}L {D['s']}S")
                    elif t == "/debug":   threading.Thread(target=run_debug, daemon=True).start()
                    elif t == "/ayuda":   send("⏰10:00|15:30|21:00 ARG\n/analizar fuerza\n/debug diagnóstico")
        except:
            pass
        time.sleep(2)

def servidor():
    port = int(os.environ.get("PORT", 8080))
    app.run(host="0.0.0.0", port=port, use_reloader=False)

def scheduler():
    ultima = ""
    while True:
        hora = datetime.now(Z).strftime("%H:%M")
        if hora in ["10:00", "15:30", "21:00"] and hora != ultima:
            ultima = hora
            run()
        time.sleep(30)

send("✅*Pump Radar v10*|Compresión H1|Zona H4|Sesgo diario")
run(forzar=True)
threading.Thread(target=listen,    daemon=True).start()
threading.Thread(target=scheduler, daemon=True).start()
threading.Thread(target=servidor,  daemon=True).start()
while True:
    time.sleep(60)