...
def choch(v,tipo):
    ...
    if tipo=="long":
        # Busca minimo decreciente seguido de ruptura de maximo previo
        prev_hi=max(Hi[:5])
        curr_hi=max(Hi[5:])
        prev_lo=min(Lo[:5])
        curr_lo=min(Lo[5:])
...
def fibonacci(Hi,Lo,Cl,tipo):
    ...
    if tipo=="long":
        ...
        return swing_hi-rango*0.705<=Cl[-1]<=swing_hi-rango*0.618
    else:
        swing_hi=max(Hi[-20:]);idx_hi=Hi[-20:].index(swing_hi)
        swing_lo=min(Lo[idx_hi:])
        if swing_lo>=swing_hi:return False
        rango=swing_hi-swing_lo
        return swing_lo+rango*0.618<=Cl[-1]<=swing_lo+rango*0.705
...
def ana(par,sym):
    ...
    # Contexto H4 y CHoCH en H1
    h4=dir_h4(par)
    v1h=ohlc(par,60)
    choch_l=choch(v1h,"long")
    choch_s=choch(v1h,"short")
    ...
    for tipo in["long","short"]:
        ...
        # H4 debe estar alineado o neutral — nunca en contra
        if tipo=="long" and h4=="b":continue
        if tipo=="short" and h4=="a":continue

        # CHoCH en H1 obligatorio — filtro principal
        if tipo=="long" and not choch_l:continue
        if tipo=="short" and not choch_s:continue
        ...
        # SCORE
        sc=30+int(sc_c*0.35)
        sc+=25 if(h4=="a"and tipo=="long")or(h4=="b"and tipo=="short")else 10
        sc+=15 if(tipo=="long"and choch_l)or(tipo=="short"and choch_s)else 0
        ...
        h4t={"a":"H4 Alc✅","b":"H4 Baj✅","n":"H4 Neu"}.get(h4,"")
        cht="CHoCH✅"if(tipo=="long"and choch_l)or(tipo=="short"and choch_s)else""
        fibt="Fib✅"if fib else""
        apal=5 if sc>=85 else 3
        tags=[f"🔇Comp:{sc_c}pts",h4t,cht]
...
def dbg(par,sym):
    ...
    h4n={"a":"alc","b":"baj","n":"neu"}.get(h4,"?")
    rn={"u":"↑","d":"↓","r":"rng"}.get(reg,"?")
    cht=f"CHoCH:{'L✅'if choch_l else''}{'S✅'if choch_s else''}{'❌'if not choch_l and not choch_s else''}"
...
def run_bg():
    ...
    msg=f"⚡*PUMP RADAR v6.1—{now.strftime('%H:%M')}ARG*\n_H4+CHoCH H1+Fib_\n\n"
    for r in tl+ts:
        s="+";sl="-"
